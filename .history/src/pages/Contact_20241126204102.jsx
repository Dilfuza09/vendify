...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setIsFlipped(true); // Переключаем состояние
    setTimeout(() => {
      setIsFlipped(false); // Возвращаемся к форме через 3 секунды
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F5F5F5] p-6">
      <div className="relative w-full max-w-md h-[500px] perspective">
        {/* Передняя сторона */}
        <div
          className={`absolute w-full h-full bg-white shadow-md rounded-lg transform transition-transform duration-700 ${
        ...
        {/* Задняя сторона */}
        <div
          className={`absolute w-full h-full bg-gray-50 shadow-md rounded-lg transform transition-transform duration-700 rotate-y-180 ${
            isFlipped ? "" : "-rotate-y-180"
